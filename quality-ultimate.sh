...
# ============================================================================
# 9️⃣ TESTS Y COBERTURA
# ============================================================================
print_header "🧪 TESTS Y COBERTURA"

# 9.1 Ejecutar tests
print_section "Ejecutando tests..."
if npm run test -- --run 2>/dev/null; then
    print_success "Tests pasando"
else
    print_error "Tests fallando"
    count_issue
fi

# 9.2 Verificar cobertura
print_section "Verificando cobertura de código..."
if npm run test:coverage -- --run 2>/dev/null; then
    if [ -f "coverage/coverage-summary.json" ]; then
        COVERAGE=$(node -e "console.log(JSON.parse(require('fs').readFileSync('coverage/coverage-summary.json', 'utf-8')).total.lines.pct)" 2>/dev/null || echo "0")
        COVERAGE_INT=$(echo "$COVERAGE" | cut -d'.' -f1)

        echo -e "       Cobertura actual: ${CYAN}${COVERAGE}%${NC} (mínimo: ${MIN_COVERAGE}%)"

        if [ "$COVERAGE_INT" -lt "$MIN_COVERAGE" ]; then
            print_error "Cobertura insuficiente"
            count_issue
        else
            print_success "Cobertura OK"
        fi
    else
        print_warning "No se generó reporte de cobertura"
    fi
else
    print_warning "Tests de cobertura no disponibles"
fi
...
# 11.2 Verificar imports ordenados
print_section "Verificando orden de imports..."
UNORDERED_IMPORTS=$(npx eslint src/ --ext .ts,.tsx \
    --rule "sort-imports: ['error', {'ignoreCase': true, 'ignoreDeclarationSort': true}]" \
    --format compact 2>&1 | grep sort-imports || true)
if [ -n "$UNORDERED_IMPORTS" ]; then
    print_warning "Imports sin ordenar:"
    echo "$UNORDERED_IMPORTS" | head -3
    count_issue
else
