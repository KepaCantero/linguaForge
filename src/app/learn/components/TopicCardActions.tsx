    ...
    <>
      ...
      {topic.type === 'imported' && (
        <motion.button
          ...
          className="absolute top-3 right-3 w-8 h-8 rounded-full bg-semantic-error/20 hover:bg-semantic-error text-semantic-error hover:text-calm-text-primary opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center text-sm"
      ...
      {showGlowEffect && (
        <motion.div
          className="absolute inset-0 rounded-2xl bg-gradient-to-r from-accent-500/20 to-accent-500/10 blur-xl opacity-0 group-hover:opacity-100 transition-opacity"
          animate={{
            opacity: [0.1, 0.2, 0.1],
          }}
