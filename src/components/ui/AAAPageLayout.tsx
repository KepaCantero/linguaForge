...
export function AAACard({
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden rounded-2xl bg-calm-bg-secondary backdrop-blur-md border border-calm-warm-100/30 shadow-calm-lg">
        ...
        <div className="relative p-5">
          ...
          <div className="flex items-start gap-4 mb-4">
            {icon && (
              <motion.div
                className="relative w-14 h-14 rounded-xl bg-gradient-to-br to-accent-500 to-sky-500 flex items-center justify-center text-3xl shadow-calm-lg"
                whileHover={{ rotate: 5, scale: 1.05 }}
                transition={{ type: 'spring', stiffness: 300 }}
              >
                <motion.div
                  className="absolute inset-0 rounded-xl bg-accent-500 blur-md opacity-50"
                  animate={{
                    scale: [1, 1.2, 1],
                    opacity: [0.3, 0.6, 0.3],
                  }}
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-bold text-lg text-white mb-1 line-clamp-1 group-hover:text-amber-500 transition-colors">
                {title}
              </h3>
              {subtitle && (
                <div className="text-sm text-calm-text-muted mb-2">
                  {subtitle}
                </div>
              )}
              {description && (
                <p className="text-sm text-calm-text-muted/80 line-clamp-2">
                  {description}
                </p>
              )}
            </div>
          ...
          {stats && stats.length > 0 && (
            <div className="flex items-center gap-4 mt-4 pt-4 border-t border-calm-warm-100/20">
              {stats.map((stat, i) => (
